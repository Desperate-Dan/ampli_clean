...
def mini_mapper(output_name, input_ref, secondary):
    ...
    os.system("minimap2 -a %s -x map-ont -o %s.bam %s binned_reads.fastq.gz | samtools sort -o %s.sorted.bam && samtools index %s.sorted.bam" % (sec, output_name, input_ref, output_name, output_name))
